...
export const Carrossel = ({ imagens }: CarrosselProps) => {
  ...
  const proximoSlide = () => {
    setSlide((slideAnterior) =>
      slideAnterior === imagens.length - 1 ? 0 : slideAnterior + 1
    );
  };

  const slideAnterior = () => {
    setSlide((slideAnterior) =>
      slideAnterior === 0 ? imagens.length - 1 : slideAnterior - 1
    );
  };
    ...
    <div>
      <CarroselIntegrantes>
        ...
        {imagens.map((item: CarrosselLista, idx: number) => (
          ...
          >
            <img className="slide" src={item.imagem} alt={`Imagem ${idx}`} />
        ...
        <span className="indicadores">
          {imagens.map((_item: CarrosselLista, idx: number) => (
            <button
              ...
              className={
                slide === idx ? "indicador" : "indicador indicador-inativo"
              }
            />
          ))}
        </span>
